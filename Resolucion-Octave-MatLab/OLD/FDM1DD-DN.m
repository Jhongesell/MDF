%{
 ...
 así, para el problema particular
  ...
  Escribir en consola
   ...
   [error,A,b,u,x,V] = fdm1d(p,q,r,f,0,1,-1,0,-1,1,11,1,s,1);
 ...
 así, para el problema particular
  ...
  Escribir en consola
   ...
   f=@(x) -pi*pi*cos(pi*x);
   s=@(x) cos(pi*x);
   [error,A,b,u,x,V] = fdm1d(p,q,r,f,0,1,-1,1,-1,-1,40,1,s,1);
  ...
  Escribir en consola
   ...
   f=@(x) -pi*pi*cos(pi*x);
   s=@(x) cos(pi*x);
   [error,A,b,u,x,V] = fdm1d(p,q,r,f,0,0.5,-1,1,-2,-pi,40,1,s,1);
 
%}



%% [error,A,b,u,x,V] = fdm1d(p,q,r,f,s,xi,xf,tf1,vf1,tf2,vf2,n,grf,s,ssw)
%{
 ...
 b   Fin del dominio
 tf1 Tipo de frontera inicial (-1) Dirichlet y (-2) Neumann
 vf1 Valor de la condicion inicial 
 tf1 Tipo de frontera final (-1) Dirichlet y (-2) Neumann
 vf2 Valor de la condicion final
...
%}
function [error,A,b,u,x,V] = fdm1d(p,q,r,f,xi,xf,tf1,vf1,tf2,vf2,n,grf,s,sws)
  if n < 3 
     return
  end
  ...
  if grf == 1
    ...
    % Graficar la solucion analitica en una particion de tamaño xPart
    if sws == 1
      xPart = 1000;
      h = (xf-xi)/(xPart-1);
      xx = zeros(xPart,1);
